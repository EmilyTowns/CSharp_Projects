...
class Program
{
    static void Main(string[] args)
    {
        ...
        if (!stringList.Contains(colourInput))   //If the list does not contain colourInput, indicates that the colour is not on the list
        {
        ...
        for (int m = 0; m < stringList2.Count; m++)   //Starts a for loop where m starts at 0, loop runs while m is less than the length of the list, adding one to m each loop  
        {
            if (stringList2[m].Equals(makeInput))   //If the value at index is equal to the user input, write the index of that item to the console
            {
        ...
        if (!stringList2.Contains(makeInput))   //If the list does not contain makeInput, states it is not on the list
        {
        ...
        List<string> stringList4 = new List<string>();   //Starts a new empty string list

        foreach (string home in stringList3)   //Starts a foreach loop, going through each item in the list  
        {
            if (!stringList4.Contains(home))   //Checks to see if stringList4 doesn't contain the home type name, writing the home type and that it is unique to the console
            {
                Console.WriteLine(home + " - this item is unique");
            }
            else   //If the house type is on stringList4, states that it is a duplicate
            {
                Console.WriteLine(home + " - this item is a duplicate");
            }
            stringList4.Add(home);   //Adds the house type to list 4 to track which house types have already been looped over so duplicates can be identified
        }
        Console.ReadLine();  //Keeps the console open
    }
}
